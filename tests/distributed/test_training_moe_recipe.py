...
def test_cli_mixtral_ep2_edp2_data_resume_standard_artifact(tmp_path, monkeypatch):
    ...
    def execute(value, name):
        ...
        try:
            ...
            pytest.fail(f"Mixtral CLI failed: stdout={error.stdout}\nstderr={error.stderr}")
        assert result.returncode == 0
        stage = read_json(tmp_path / name / "stage.json")
        assert stage["status"] == "complete"
        assert stage["result"]["details"]["global_batch_size"] == 8
        assert stage["result"]["details"]["parallel"]["expert_parallel"] == 2
        assert stage["result"]["details"]["parallel"]["data_parallel"] == 4
        ...
        for rank, entry in enumerate(checkpoint["entries"]):
            ...
            assert state["rank"] == rank and state["world_size"] == 4
